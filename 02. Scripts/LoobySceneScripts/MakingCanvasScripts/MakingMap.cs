...
public class MakingMap : MonoBehaviour
{
    ...
    private void Start() {
        ...
        data.mapes.Add(new SaveData.MapData(buttons[0], "맵 1번에 관한 설명입니다."));
        data.mapes.Add(new SaveData.MapData(buttons[1], "맵 2번에 관한 설명입니다."));
        ...
        if(File.Exists(filePath))
        {
            ...
            for(int i = 0; i < data.mapes.Count; i++ )
            {
                ...
                if(data.mapes[i].id == selectItem)
                {
                    ...
                    lobbySceneMananger.mapImage = selectItem.image.sprite;
                }
            }
        }
        // 파일이 없을 때.
