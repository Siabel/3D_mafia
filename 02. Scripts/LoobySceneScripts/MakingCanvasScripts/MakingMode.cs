...
{
    public LobbySceneManager lobbySceneMananger;
    ...
    private void Start() {
        ...
        data.modes.Add(new SaveData.ModeData(buttons[0], "모드 1번에 관한 설명입니다."));
        data.modes.Add(new SaveData.ModeData(buttons[1], "모드 2번에 관한 설명입니다."));
        ...
        if(File.Exists(filePath))
        {
            ...
            for(int i = 0; i < data.modes.Count; i++ )
            {
                ...
                if(data.modes[i].id == selectItem)
                {
                    ...
                    lobbySceneMananger.modeImage = selectItem.image.sprite;
                }
            }
        }
        // 파일이 없을 때.
